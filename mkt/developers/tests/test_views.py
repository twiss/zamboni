...
class TestMarketplace(amo.tests.TestCase):
    ...
    def setUp(self):
        ...
        self.addon.form_factor_set.create(form_factor_id=mkt.FORM_DESKTOP.id)
    ...
    def get_data(self, **kw):
        data = {
            ...
            'platform': [mkt.PLATFORM_DESKTOP.id],
            'form_factor': [mkt.FORM_DESKTOP.id],
            'app_type': 'hosted',
            'payment': 'paid',
            'price': self.price.pk,
    ...
    def test_initial_free(self):
        self.addon.platform_set.all().delete()
    ...
    def test_change_form_factor(self):
        self.setup_premium()
        res = self.client.post(
            self.url, data=self.get_data(form_factor=[mkt.FORM_DESKTOP.id,
                                                      mkt.FORM_MOBILE.id]))
        eq_(res.status_code, 302)
