...
class UploadAddon(object):
    ...
    def post(self, expect_errors=False, data=None):
        defaults = {'app_type': 'hosted', 'payment': 'free',
                    'platform': [mkt.PLATFORM_FXOS.id],
...
class TestCreateWebApp(BaseWebAppTest):
    ...
    def test_no_upload(self):
        data = {'app_type': 'packaged', 'payment': 'free',
                'platform': [mkt.PLATFORM_FXOS.id],
                'form_factor': [mkt.FORM_MOBILE.id]}
    ...
    def test_bad_upload(self, fi_mock):
        data = {'app_type': 'packaged', 'payment': 'free',
                'platform': [mkt.PLATFORM_FXOS.id],
                'form_factor': [mkt.FORM_MOBILE.id], 'upload': 'foo'}
        res = self.client.post(self.url, data, follow=True)
        eq_(res.context['form'].errors,
            {'upload': NewWebappVersionForm.upload_error})
    ...
    def test_app_from_uploaded_manifest(self):
        ...
        eq_(addon.platforms, [mkt.PLATFORM_FXOS])
        eq_(addon.form_factors, [mkt.FORM_MOBILE])
    ...
    def test_set_platform(self):
        app = self.post_addon(
            {'platform': [mkt.PLATFORM_ANDROID.id, mkt.PLATFORM_DESKTOP.id]})
        self.assertSetEqual(app.platforms,
                            [mkt.PLATFORM_ANDROID, mkt.PLATFORM_DESKTOP])

    def test_free(self):
        app = self.post_addon({'payment': 'free'})
    ...
    def test_appfeatures_creation(self):
        addon = self.post_addon({'has_contacts': 'on'})
